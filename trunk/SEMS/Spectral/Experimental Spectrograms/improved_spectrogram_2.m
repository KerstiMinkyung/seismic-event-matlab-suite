function improved_spectrogram(wave,nf_rng)
%
%IMPROVED_SPECTROGRAM: Sliding hanning windowed fft
%
...
for n = 1:spg_w
   for m = 1:spg_h
      ...
      win_v = v_b(d(n)-win_w(m):d()+win_w(m));
      f_rad = f(n)/Fs*2*pi;
      coeff = 2*cos(f_rad);
   ...
   fft_array(n,m) = s_prev2*s_prev2 + s_prev*s_prev - coeff*s_prev2*s_prev;
   end
end
warning on
figure
