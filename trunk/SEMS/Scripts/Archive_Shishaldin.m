...
%% TIME RANGE
log.start = floor(datenum([2010 1 1 0 0 0]));
log.end = ceil(now);
   ...
   for m = 1:numel(log.scnl)  % LOOP OVER SCNL
      ...
      if ~isempty(w)
         ...
         if ~isempty(wfa_day)
            ...
            metric_day.start = get(wfa_day,'start')
         ...
         disp(['No waveform available for ', get(log.scnl(m),'station'),...
            ':', get(log.scnl(m),'channel'),' on ',datestr(log.day(n))])
      end
   end
end
